...
#define SysTick    ( (SysTick_Type *) 0xE000E010UL)   /*!< SysTick configuration struct */


/*@} end of group CMSIS_SysTick */

/*
 * SysTick con campo de bits
 * **/
typedef struct
{
	union{
		...
		struct{
			...
			__IO uint32_t RESERVED0:13; // bits 3 al 15
			__IO uint32_t COUNTFLAG:1;
			__IO uint32_t RESRVED1:15; // bits 17 al 31
		}bits;
	};
  __IO uint32_t SYST_RVR;                   /*!< Offset: 0x000 (R/W)  SysTick Control and Status Register */
  __IO uint32_t SYST_CVR;                   /*!< Offset: 0x004 (R/W)  SysTick Reload Value Register */
  __I uint32_t SYST_CALIB;                    /*!< Offset: 0x008 (R/W)  SysTick Current Value Register */
} SysTick_t;
...
/**
 * Fin SysTick con campos de bits
 * */
...
// Macro de lso bits individuales
#define NVIC_PININT0_BIT 24U
